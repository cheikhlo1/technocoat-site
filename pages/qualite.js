...
import { renderKpiCard, renderEmptyState, renderSeverityBadge, renderStatusBadge, renderActionButton, renderAlertCard, renderProgressBar, renderComparisonBars, renderDonutChart, renderMiniBarChart } from '../components/charts.js';
...
function kpiHtml(section) {
  return `<div class="table-grid logistic-kpi-grid">${sectionKpis(section).map(([l, v]) => renderKpiCard(l, v)).join('')}</div>`;
}

function sectionEquipements(showForm = false) {
  const { bains, fours, cabines, appareils } = equipementData();
  const conformes = bains.filter((b) => String(b.resultat).includes('Conforme')).length;
  const nonConformes = bains.filter((b) => /Non conforme|Alerte/i.test(String(b.resultat))).length;
  return `<article class="card"><h4>Contrôle équipements</h4><p>Suivi des contrôles bains, maintenance fours et appareils de mesure.</p>${kpiHtml('equipements')}<div class="table-grid">${renderDonutChart('Taux conformité bains', conformes, Math.max(1, conformes + nonConformes))}${renderComparisonBars([{ label: 'Conformes', value: conformes }, { label: 'Non conformes', value: nonConformes }])}</div>
...
function sectionSignalements(state) {
  ...
  const row = (o) => `<tr><td>${safe(o.dateObservation)}</td><td>${safe(o.origineSignalement)}</td><td>${safe(o.client)}</td><td>${safe(o.numeroOF)}</td><td>${safe(o.reference)}</td><td>${safe(o.designation)}</td><td>${safe(o.typeDefaut)}</td><td>${renderSeverityBadge(o.gravite)}</td><td>${safe(o.quantiteConcernee)}</td><td>${renderStatusBadge(o.statutTraitement)}</td><td>${safe(o.responsableTraitement)}</td><td>${safe(o.actionPrevue || '')}</td><td>${renderActionButton('Détail', 'secondary', `data-s-action="detail" data-id="${o.id}"`)} ${renderActionButton('Modifier', 'secondary', `data-s-action="edit" data-id="${o.id}"`)}</td></tr>`;
  ...
  if (state.mode === 'detail' || state.mode === 'edit' || state.mode === 'create') {
    ...
    const detailBlocks = state.mode === 'detail' ? `<div class="table-grid">${renderDetailBlock('Identification', [{ label: 'Date', value: current.dateObservation }, { label: 'Origine', value: current.origineSignalement }, { label: 'Client', value: current.client }, { label: 'N° OF', value: current.numeroOF }, { label: 'Référence', value: current.reference }, { label: 'Désignation', value: current.designation }])}${renderDetailBlock('Problème', [{ label: 'Type défaut', value: current.typeDefaut }, { label: 'Gravité', value: current.gravite }, { label: 'Quantité concernée', value: current.quantiteConcernee }, { label: 'Description', value: current.descriptionProbleme }])}${renderDetailBlock('Analyse', [{ label: 'Cause probable', value: current.causeProbable }, { label: 'Responsable traitement', value: current.responsableTraitement }, { label: 'Statut', value: current.statutTraitement }])}${renderDetailBlock('Action corrective', [{ label: 'Action prévue', value: current.actionPrevue }, { label: 'Date cible', value: current.dateCibleCloture }, { label: 'Date clôture', value: current.dateCloture || '-' }, { label: 'Commentaire clôture', value: current.commentaireCloture || '-' }])}</div>` : '';
...
function sectionStandards() {
  ...
  const actifs = standards.filter(([, , s]) => s === 'Actif').length;
  const maj = standards.filter(([, , s]) => s === 'À mettre à jour').length;
  return `<article class="card"><h4>Standards process</h4><p>Suivi des standards qualité, des validations et des écarts documentaires.</p>${kpiHtml('standards')}<div class="table-grid">${renderProgressBar('Standards actifs', actifs, Math.max(1, standards.length))}${renderComparisonBars([{ label: 'Actifs', value: actifs }, { label: 'À mettre à jour', value: maj }])}</div><div class="table-grid">${standards.map(([n, o, s]) => `<article class="card"><h5>${n}</h5><p>${o}</p><p><strong>Statut :</strong> ${s}</p></article>`).join('')}</div></article>`;
}
