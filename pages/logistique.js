...
function render(container, state = {}) {
  const { section = 'reception', receptionKpi = null, receptionDetail = null, receptionEdit = null, receptionRefs = 1, shipDetail = null, shipEdit = null, shipObserve = null } = state;
  const all = buildRows();
  ...
  <div class="table-grid logistic-kpi-grid"><article class="card kpi"><h4>Expéditions prévues aujourd’hui</h4><p class="kpi-count">${sk.prevues}</p></article><article class="card kpi"><h4>Affaires prêtes à expédier</h4><p class="kpi-count">${sk.pretes}</p></article><article class="card kpi"><h4>Expéditions réalisées</h4><p class="kpi-count">${sk.realisees}</p></article><article class="card kpi"><h4>Expéditions en retard</h4><p class="kpi-count">${sk.retard}</p></article><article class="card kpi"><h4>BL à préparer</h4><p class="kpi-count">${sk.bl}</p></article><article class="card kpi"><h4>Demandes de ramassage à prévoir</h4><p class="kpi-count">${sk.ramassage}</p></article></div>
  ...
  container.querySelectorAll('[data-ship-detail]').forEach((b) => b.addEventListener('click', () => { const id = Number(b.dataset.shipDetail); render(container, { ...state, section: 'shipping', shipDetail: shipDetail === id ? null : id, shipEdit: false, shipObserve: false }); }));
  ...
  container.querySelectorAll('[data-ship-action]').forEach((b) => b.addEventListener('click', () => {
    ...
    render(container, { ...state, section: 'shipping' });
  }));
...
export function renderLogistiquePage(container) { render(container, { section: 'reception', receptionRefs: 1 }); }
